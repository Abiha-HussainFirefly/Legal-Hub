...
async function seedLawyers() {
  ...
  const roleNames = ['LawyerUser', BUSINESS_ROLE_TO_TECHNICAL_ROLE.LawyerUser];
  const hash = await bcrypt.hash(LAWYER_PASSWORD, 12);
  ...
  for (const l of DEMO_LAWYERS) {
    ...
    if (existing) {
      const u = await prisma.$transaction(async (tx) => {
        ...
        await ensureUserRoles(tx, existing.userId, roleNames);
    ...
    const user = await prisma.$transaction(async (tx) => {
      ...
      await ensureUserRoles(tx, u.id, roleNames);
      const username = makeUsername(l.name);
  ...
  console.log(`✓ ${users.length} lawyer accounts seeded (password: ${LAWYER_PASSWORD})`);
  return users;
}
